...
public:
  ...
  void initialiseBuffer(Wavetable &wavetable, float frequency,
                        float phaseShift) {
  ...
  void processStatefully() {
    TypedSignalBuffer &f = *inputs[0].buffer, &out = *outputs[0].buffer;
  ...
  template <typename Frequency> void process(Frequency &f, StereoBuffer &out) {
    osc1.process(f, out);
